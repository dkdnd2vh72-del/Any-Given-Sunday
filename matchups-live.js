(function () {
  ...
  style.textContent = ".matchup-expand-row{display:flex;justify-content:center;margin-top:10px;padding-top:10px;border-top:1px solid rgba(133,171,203,.12)}.matchup-expand-btn{display:flex;align-items:center;gap:7px;min-height:38px;padding:6px 13px;border:1px solid rgba(133,171,203,.2);border-radius:999px;background:rgba(255,255,255,.025);color:#9db1c2;font-size:.68rem;font-weight:850;cursor:pointer}.matchup-expand-btn .chevron{font-size:1rem;line-height:1;transition:transform .18s ease}.matchup-expand-btn[aria-expanded=true]{color:var(--lime);border-color:rgba(201,255,54,.28);background:rgba(201,255,54,.06)}.matchup-expand-btn[aria-expanded=true] .chevron{transform:rotate(180deg)}.matchup-rosters{display:grid;grid-template-columns:1fr;gap:12px;margin-top:12px;padding-top:12px;border-top:1px solid rgba(133,171,203,.12)}.matchup-roster-team{min-width:0;padding:12px;border:1px solid rgba(133,171,203,.15);border-radius:14px;background:rgba(2,13,25,.45)}.matchup-roster-title{text-align:center;font-size:.78rem;font-weight:900;margin-bottom:8px;color:#eaf4fb}.matchup-starter-row{display:grid;grid-template-columns:38px 1fr auto;align-items:center;gap:8px;min-height:34px;border-top:1px solid rgba(133,171,203,.09);font-size:.72rem}.matchup-starter-row:first-of-type{border-top:0}.matchup-starter-slot{color:var(--lime);font-size:.6rem;font-weight:900}.matchup-starter-name{min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.matchup-starter-score{font-weight:900;color:#9be7ff}.matchup-roster-total{display:flex;justify-content:space-between;align-items:center;margin-top:7px;padding-top:9px;border-top:1px solid rgba(201,255,54,.22);color:var(--lime);font-size:.78rem;font-weight:950}@media(min-width:560px){.matchup-rosters{grid-template-columns:1fr 1fr}}";
  ...
  function makeRosterPanel(side,teamData) {
    ...
    if (!starters.length) {
      var empty=document.createElement("div"); empty.className="matchup-projection"; empty.style.textAlign="center"; empty.textContent="Starting lineup unavailable"; panel.appendChild(empty);
    } else {
  ...
  function makeCard(matchup,teamMap,key) {
    ...
    var isOpen=openMatchupKey===key; button.setAttribute("aria-expanded",isOpen?"true":"false"); button.innerHTML='<span class="chevron">⌄</span><span>Starting rosters</span>'; expandRow.appendChild(button); card.appendChild(expandRow);
